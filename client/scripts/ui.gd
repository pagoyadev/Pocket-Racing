...
const PREVIEW_CAR_COLOR := Color(0.85, 0.22, 0.22)
...
func _update_car_preview() -> void:
	...
	var body_mesh := _car_preview_node.find_child(model_def["body_mesh"], true, false) as MeshInstance3D
	if body_mesh:
		body_mesh.set_surface_override_material(0, Game.make_body_material(PREVIEW_CAR_COLOR))
...
func add_player_to_lobby(player_name: String, color: Color = Color(0.3, 0.6, 1.0)) -> void:
